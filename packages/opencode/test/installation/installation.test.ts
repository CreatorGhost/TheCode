...
describe("installation", () => {
  describe("methodFromExecPath", () => {
    test("recognizes DCode binaries in any install directory", () => {
      expect(Installation.methodFromExecPath("/home/user/bin/dcode")).toBe("curl")
      expect(Installation.methodFromExecPath("C:\\Users\\user\\bin\\dcode.exe")).toBe("curl")
      expect(Installation.methodFromExecPath("/usr/local/bin/bun")).toBe("unknown")
    })
  })
  ...
  describe("upgrade", () => {
    ...
    ).effect("rejects unsupported installation methods", () =>
      Effect.gen(function* () {
        const error = yield* Effect.flip(Installation.use.upgrade("unknown", "9.9.9"))
        expect(error).toBeInstanceOf(Installation.UpgradeFailedError)
        expect(error.stderr).toBe("Unknown installation method: unknown")
    ...
    testEffect(
      ...
      Effect.gen(function* () {
        ...
        expect(installerCalls).toContain("https://raw.githubusercontent.com/CreatorGhost/TheCode/v9.9.9/install")
      }),
    )
