...
export function userAgent(client = "cli") {
  return `dcode/${InstallationChannel}/${InstallationVersion}/${client}`
}
...
export function methodFromExecPath(execPath: string): Method {
  if (["dcode", "dcode.exe"].includes(path.basename(execPath.replaceAll("\\", "/")).toLowerCase())) return "curl"
  return "unknown"
}
  ...
  Effect.gen(function* () {
    ...
    const upgradeCurl = Effect.fnUntraced(
      function* (target: string) {
        const response = yield* httpOk.execute(
          HttpClientRequest.get(`https://raw.githubusercontent.com/CreatorGhost/TheCode/v${target}/install`),
        )
    ...
    const result: Interface = {
      ...
      method: Effect.fn("Installation.method")(function* () {
        return methodFromExecPath(process.execPath)
      }),
      ...
      upgrade: Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        if (m !== "curl") return yield* new UpgradeFailedError({ stderr: `Unknown installation method: ${m}` })
