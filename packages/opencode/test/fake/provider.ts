    ...
    return {
      ...
      layer: Layer.succeed(
        ...
        Provider.Service.of({
          ...
          resolveModel: Effect.fn("TestProvider.resolveModel")((selector) => {
            if (selector === mdl.name || selector === `${row.id}/${mdl.id}`) return Effect.succeed(mdl)
            return Effect.die(new Error(`Unknown test model selector: ${selector}`))
          }),
